...
public:
  ...
  : rclcpp::Node("offboard_controller")
  {
    ...
    status_sub_ = this->create_subscription<airship_msgs::msg::AirshipStatus>(
      ...
      std::bind(&OffboardController::on_status, this, _1));
    ...
    target_vel_sub_ = this->create_subscription<geometry_msgs::msg::TwistStamped>(
      "/airship/cmd/target_velocity", rclcpp::QoS(10),
    ...
    debug_pub_ = this->create_publisher<std_msgs::msg::String>(
      "/airship/control/debug", rclcpp::QoS(10));
    ...
    RCLCPP_INFO(this->get_logger(),
      ...
      control_mode_.c_str(), control_rate_, alt_min_, alt_max_, yaw_rate_limit_);
  }
...
private:
  ...
  void timer_control()
  {
    ...
    if (control_mode_ == "position" && has_target_pos_) {
      ...
      float clamped_alt = clampf(target_alt, static_cast<float>(alt_min_), static_cast<float>(alt_max_));
      if (std::abs(clamped_alt - target_alt) > 0.01f) {
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
          "目标高度 %.1fm 超出限位[%.0f,%.0f], 已限制为 %.1fm",
          target_alt, alt_min_, alt_max_, clamped_alt);
      }
      ...
      // 速度限幅
      float vx = clampf(target_vel_[0], -static_cast<float>(max_horiz_vel_), static_cast<float>(max_horiz_vel_));
      float vy = clampf(target_vel_[1], -static_cast<float>(max_horiz_vel_), static_cast<float>(max_horiz_vel_));
      float vz = clampf(target_vel_[2], -static_cast<float>(max_vert_vel_), static_cast<float>(max_vert_vel_));
      ...
      if (current_alt >= alt_max_ && vz < 0) {
        // 已达上限, 禁止继续上升(vz<0=上升)
        vz = 0.0f;
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "已达高度上限, 禁止上升");
      }
      if (current_alt <= alt_min_ && vz > 0) {
        // 已达下限, 禁止继续下降(vz>0=下降)
        vz = 0.0f;
        RCLCPP_WARN_THROTTLE(this->get_logger(), *this->get_clock(), 2000, "已达高度下限, 禁止下降");
      }
