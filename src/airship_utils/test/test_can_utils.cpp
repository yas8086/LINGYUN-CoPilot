...
// ===== 小端序整数解析 =====
TEST(CanUtilsTest, U16LittleEndian)
{
...
TEST(CanUtilsTest, U32LittleEndian)
{
...
TEST(CanUtilsTest, I16LittleEndian)
{
  // 0xFFFE = -2 (补码)
...
TEST(CanUtilsTest, TempOffset)
{
  ...
  // 过温区间: raw 为无符号字节, raw=140 -> 100℃(旧 int8_t 实现曾截断为 -156℃,
  // 过温告警反向失效——此用例锁定修复)
  EXPECT_FLOAT_EQ(temp_with_offset(140), 100.0f);
  EXPECT_FLOAT_EQ(temp_with_offset(200), 160.0f);
}

// ===== 边界: 模拟 DCDC 开机帧整体解析 =====
// 开机 + 48.0V + 80.0A: Data = 00 00 10 E0 01 20 03 00
TEST(CanUtilsTest, DcdcPowerOnFrame)
{
