...
|--------|-------------|------|------|------|------|
| roll | `/airship/mavros/attitude/roll` | float32 | deg | 0.1 | 横滚角(不可控,仅监视) |
...
| 参数名 | MAVROS Topic | 类型 | 说明 |
|--------|-------------|------|------|
| actuator_outputs | `/airship/mavros/actuator_control/outputs` | float32[8] | 8个电机归一化输出[0,1] |
...
|------|------|---------|---------|---------|------|
| roll | deg | > 20 | > 30 | > 45 | 飞艇无Roll控制, 异常偏大=失稳 |
...
| mode | string | - | - | 飞行模式名称 |
| m0-m7 | float32 | [0,1] | 0.001 | 8个电机归一化输出 |
...
2. **Roll 告警阈值应宽松** — 飞艇无 Roll 控制, 依赖气动自稳定, 10-20 度 Roll 可能正常
